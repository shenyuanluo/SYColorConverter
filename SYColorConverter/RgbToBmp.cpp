...
bool RgbToBmp::Rgb565ToBmp(unsigned char *inRgb, unsigned int width, unsigned int height, const char *outBmpPath) const
{
...
bool RgbToBmp::Rgb24ToBmp(unsigned char *inRgb, unsigned int width, unsigned int height, const char *outBmpPath) const
{
    if (NULL == inRgb || NULL == outBmpPath || 0 == width || 0 == height)
    {
        return false;
...
#pragma mark - Private
#pragma mark -- 检查大小端
EndianType RgbToBmp::CheckEndian()
{
    union {
        char c[4];
        unsigned int num;
