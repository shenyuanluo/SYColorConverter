...
YuvToBmp::YuvToBmp()
{
    ...
    m_eType = CheckEndian();
    m_bmpHeaderLen   = sizeof(BitMapHeader);
    m_bmpFileInfoLen = sizeof(BitMapFileInfo);
...
void YuvToBmp::setConvertType(ConvertType cType)
{
    m_cType = cType;
}

#pragma mark -- 设置转换矩阵类型
void YuvToBmp::setMatrixType(MatrixType mType)
{
    m_mType = mType;
}
...
bool YuvToBmp::I420ToBmp(unsigned char* inYuv, unsigned int width, unsigned int height, const char* outBmpPath) const
{
    ...
    BitMapHeader   m_BMPHeader   = {0};     // BMP 头结构
    BitMapFileInfo m_BMPFileInfo = {0};     // BMP 文件信息
...
bool YuvToBmp::Nv12ToBmp(unsigned char* inYuv, unsigned int width, unsigned int height, const char* outBmpPath) const
{
    if (inYuv == NULL || outBmpPath == NULL || 0 == width || 0 == height)
    {
        return false;
...
bool YuvToBmp::Nv21ToBmp(unsigned char* inYuv, unsigned int width, unsigned int height, const char* outBmpPath) const
{
    if (inYuv == NULL || outBmpPath == NULL || 0 == width || 0 == height)
    {
        return false;
...
#pragma mark - Private
#pragma mark -- 检查大小端
EndianType YuvToBmp::CheckEndian()
{
    union {
        char c[4];
        unsigned int num;
...
void YuvToBmp::yuv2rgb(int y, int u, int v, int* r, int* g, int* b) const
{
    switch (m_mType)
    {
        case Matrix_normal:
